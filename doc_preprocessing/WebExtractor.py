...
# opt = np.load(osp.join(DATA_DIR, "options.npy"), allow_pickle=True).item() 
# options = opt
# print(f"original options quiet {options.quiet}, debug {options.debug}")
options.quiet=True
options.debug=False
##
...
class Extractor(object):
    ...
    def __init__(self, id, revid, title, lines, url, keep_url=False):
        """
        :param id: id of page.
...
def my_extract(input):
    for line in input:
        line_dict = json.loads(line)
        title = line_dict['Title']
        page = line_dict['Primary']
        lang = line_dict['Language']
        url = line_dict['Url']
        yield title, page, lang, url
...
def process_dump(input_file, out_file, file_size, file_compress,
                 process_count, keep_url=False, url_subset_file=None):
    """
    ...
    # outfile = open(os.path.join(DATA_DIR, "options.json"), 'w')
    ...
    for i in range(worker_count):
        extractor = Process(target=extract_process,
                            args=(options, i, jobs_queue, output_queue, keep_url))
    ...
    for page_data in my_extract(input):
        ...
        job = (0, 0, title, page, url, page_num)
...
def extract_process(opts, i, jobs_queue, output_queue, keep_url=False):
    ...
    while True:
        job = jobs_queue.get()  # job is (id, title, page, url, page_num)
        if job:
            id, revid, title, page, url, page_num = job
            try:
                e = Extractor(*job[:5], keep_url=keep_url)  # (id, revid, title, url, page)
                page = None  # free memory
                e.extract(out)
                text = out.getvalue()
            except:
...
def reduce_process(opts, output_queue, spool_length,
    ...
    while True:
        if next_page in spool:
            pp = spool.pop(next_page)
            try:
                # output.write(spool.pop(next_page).encode('utf-8'))
                output.write(pp.encode('utf-8'))
            except:
                logging.error(f"error write {pp}")
            ...
            if next_page % report_period == 0:
                ...
                logging.warning("Extracted %d articles (%.1f art/s)",
...
def main():
    ...
    groupP.add_argument("--keep_url", action="store_true",
                            help="keep the url in the output article text")
    groupP.add_argument("--url_subset_file", default=None)
    ...
    # import numpy as np
    # import os.path as osp
    # opt = np.load(osp.join(DATA_DIR, "options.npy")).item() 
    # options = opt
    ...
    process_dump(input_file, output_path, file_size, args.compress, args.processes, args.keep_url, args.url_subset_file)
