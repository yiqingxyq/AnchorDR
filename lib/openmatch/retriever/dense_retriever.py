...
class Retriever:
    ...
    def doc_embedding_inference(self):
        ...
        logger.info(f"writing cached embeddings to {self.args.output_dir}")
    ...
    def init_index_and_add(self, partition: str = None):
        ...
        for i, part in tqdm(enumerate(partitions), desc=f"initializing index from {len(partitions)} partitions:"):
            ...
            if i == 0: # first figure out dim of loaded vectors
                ...
                # if self.args.use_gpu: # do this once
                #     logger.info("moving faiss index to gpu using IndexReplica scheme (exact copy on each gpu)")
        ...
        logger.info(f"Finished constructing index of {self.index.ntotal} documents w/ dim={dim}")
    ...
    def query_embedding_inference(self, query_dataset: IterableDataset):
        ...
        ### NOTE: encoded vectors must be contiguous and type float32
        encoded = np.ascontiguousarray(np.concatenate(encoded), dtype="float32")
    ...
    def search(self, topk: int = 100):
        ...
        for i in range(self.args.world_size):
            with open(os.path.join(self.args.output_dir, "embeddings.query.rank.{}".format(i)), 'rb') as f:
                data = pickle.load(f)
            lookup_indices = data[1]
            encoded.append(data[0].astype('float32'))
