...
from .tokenization_utils import Dictionary
...
TNLRv5_VOCAB_FILES_NAMES = {"vocab_file": "sp.model", "dict_file": "dict.txt"}

TNLRv5_PRETRAINED_VOCAB_FILES_MAP = {
    "vocab_file": {
        "tnlrv5-cased": "https://turingdatashare.blob.core.windows.net/projectalexander/languagemodels/tnlrv5/vocab/sp.model",
    },
    "dict_file": {
        "tnlrv5-cased": "https://turingdatashare.blob.core.windows.net/projectalexander/languagemodels/tnlrv5/vocab/dict.txt",
    }
}

TNLRv5_PRETRAINED_POSITIONAL_EMBEDDINGS_SIZES = {
    "tnlrv5-cased": 512,
    "tnlrv5-32k-uncased": 512
}

class TNLRv5Tokenizer(PreTrainedTokenizer):
    vocab_files_names = TNLRv5_VOCAB_FILES_NAMES
    pretrained_vocab_files_map = TNLRv5_PRETRAINED_VOCAB_FILES_MAP
    max_model_input_sizes = TNLRv5_PRETRAINED_POSITIONAL_EMBEDDINGS_SIZES
    ...
    def encode(self, x: str, add_special_tokens=True, max_length=512):
        tokens = self.convert_tokens_to_ids(self.tokenize(x.lower()))
        tokens = tokens[:max_length-2]
        tokens = [self.dictionary.bos_index] + tokens + [self.dictionary.eos_index]
        return tokens + [self.dictionary.pad_index] * (max_length - len(tokens))
