...
from transformers.models.bert.modeling_bert import ACT2FN, BERT_PRETRAINED_MODEL_ARCHIVE_MAP
from transformers.models.roberta.modeling_roberta import ROBERTA_PRETRAINED_MODEL_ARCHIVE_MAP
from transformers.file_utils import WEIGHTS_NAME
from transformers.activations import get_activation

from .configuration_tnlrv5 import TNLRv5Config
from .convert_state_dict import state_dict_convert, get_checkpoint_from_transformer_cache
...
TNLRv5_PRETRAINED_MODEL_ARCHIVE_MAP = {
    'tnlrv5-base-cased': "https://turingdatashare.blob.core.windows.net/projectalexander/languagemodels/tnlrv5/base/ckpt/tnlrv5_base.pt",
    'tnlrv5-large-cased': "https://turingdatashare.blob.core.windows.net/projectalexander/languagemodels/tnlrv5/large/ckpt/tnlrv5_large.pt",
}

class TNLRv5PreTrainedModel(PreTrainedModel):
    """ An abstract class to handle weights initialization 
        and a simple interface for dowloading and loading pretrained models.
    """
    config_class = TNLRv5Config
    supported_convert_pretrained_model_archive_map = {
        ...
        "electra": ELECTRA_PRETRAINED_MODEL_ARCHIVE_MAP,
        "roberta": ROBERTA_PRETRAINED_MODEL_ARCHIVE_MAP,
        "tnlrv5": TNLRv5_PRETRAINED_MODEL_ARCHIVE_MAP,
    }
    base_model_prefix = "tnlrv5"
    ...
    def from_pretrained(
        ...
        model_type = kwargs.pop('model_type', 'tnlrv5')
    ...
    def __init__(self, config):
        ...
        if hasattr(config, 'num_ffn_layers') and config.num_ffn_layers > 1:
            ...
            self.intermediate = nn.ModuleList(
                [BertIntermediate(config) for _ in range(config.num_ffn_layers)])
            self.output = nn.ModuleList(
                [BertOutput(config) for _ in range(config.num_ffn_layers)])
        else:
        ...
        if self.layer_norm_type in ('pre', 'hybrid'):
            if self.num_ffn_layers > 0:
                self.LayerNorm = nn.ModuleList([BertLayerNorm(
...
class TNLRv5Model(TNLRv5PreTrainedModel):
    r"""
    ...
    def __init__(self, config):
        super(TNLRv5Model, self).__init__(config)
        self.config = config
...
class TNLRv5ClassificationHead(nn.Module):
...
class TNLRv5ForSequenceClassification(TNLRv5PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.num_labels = config.num_labels

        self.bert = TNLRv5Model(config)
        self.classifier = TNLRv5ClassificationHead(config)
...
class TNLRv5ForQuestionAnswering(TNLRv5PreTrainedModel):
    r"""
    ...
    def __init__(self, config):
        super(TNLRv5ForQuestionAnswering, self).__init__(config)
        self.num_labels = config.num_labels
