...
class TransformerModel(TransformerModelBase):
    ...
    def load_state_dict(
        ...
        from_metro = any(k.startswith("generator.") for k in state_dict.keys())
        if from_metro:
            new_state_dict = {
                k: v
                for k, v in state_dict.items()
                if (
                    not k.startswith("generator.")
                    and not k.startswith("rtd_head.")
                )
            }
...
def tiny_architecture(args):
    ...
    args.encoder_layers = safe_getattr(args, "encoder_layers", 2)
    args.encoder_attention_heads = safe_getattr(args, "encoder_attention_heads", 2)
...
def transformer_iwslt_de_en(args):
    ...
    args.encoder_attention_heads = safe_getattr(args, "encoder_attention_heads", 4)
    args.encoder_layers = safe_getattr(args, "encoder_layers", 6)
    args.decoder_embed_dim = safe_getattr(args, "decoder_embed_dim", 512)
    args.decoder_ffn_embed_dim = safe_getattr(args, "decoder_ffn_embed_dim", 1024)
    args.decoder_attention_heads = safe_getattr(args, "decoder_attention_heads", 4)
    args.decoder_layers = safe_getattr(args, "decoder_layers", 6)
    base_architecture(args)
...
def transformer_wmt_en_de_big_t2t(args):
    ...
    args.decoder_normalize_before = safe_getattr(args, "decoder_normalize_before", True)
    args.attention_dropout = safe_getattr(args, "attention_dropout", 0.1)
